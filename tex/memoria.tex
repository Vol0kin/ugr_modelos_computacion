...
int main(int argc, char *argv[]) {
	...
	\subsection{Práctica 4}
			...
			\begin{tikzpicture}[node distance=4cm]
				...
				\draw	(q0) edge[loop below]
							node{$ \begin{array}{c}
									...
									<0,X>/XX
									\end{array}$} (q0)
						(q0) edge[above] node{$<1,X>/\varepsilon$} (q1)
						(q1) edge[loop below]
							node{$ \begin{array}{c}
									<1,X>/ \varepsilon \\
									<1,R>/YR \\
									<1,Y>/YY
								   \end{array}$} (q1)
						(q1) edge[above] node{$<0,Y>/\varepsilon$} (q2)
						(q2) edge[loop below]
							node{$ \begin{array}{c}
								   <0,Y>/\varepsilon \\
								   <1,R>/\varepsilon
		...
		\begin{itemize}
			\item $q_0$: recibe la secuencia de símbolos 0 del inicio y pone símbolos $X$ en la pila para recordar cuántos
			0 ha procesado. Cuando le llega el primer 1, pasa al estado $q_1$ y elimina una $X$ del tope de la pila.
			\item $q_1$: recibe la secuencia de símbolos 1. Por cada 1 que llegue, mientras hayan símbolos $X$ en la pila,
			se van quitando. Si se llega al símbolo de inicio de pila $R$ se empiezan a añadir símbolos $Y$ al tope de la
			pila. Se pasa al estado $q_2$ cuando llegue un 0.
			\item $q_2$: recibe la segunda secuencia de símbolos 0 y el símbolo 1 final. Por cada 0 que le llega quita
			un símbolo $Y$ de la pila. Si la pila contiene al elemento inicial al final y le llega un 1, se acepta la palabra.
			En caso contrario, se rechaza.
		\end{itemize}
		
		Una vez dicho esto, vamos a ver como quedarían las transiciones:
